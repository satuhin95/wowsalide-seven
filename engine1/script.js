...
function ws_seven(m, A, o) {
    ...
    for (var v = 0; v < d * z; v++) {
        ...
        p((J[v] = p("<div>")[0]))
            .css({ position: "absolute", overflow: "hidden", transform: "translate3d(0,0,0)" })
            .appendTo(e)
            .append(p("<img>").css({ position: "absolute", transform: "translate3d(0,0,0)" }));
        p((y[v] = p("<div>")[0]))
            .css({ position: "absolute", overflow: "hidden", transform: "translate3d(0,0,0)" })
            .appendTo(B)
            .append(p("<img>").css({ position: "absolute", transform: "translate3d(0,0,0)" }));
    ...
    function u(U, i, j, M, W) {
        ...
        p(U).each(function (af) {
            ...
            if (m.support.transform) {
                var ag = { opacity: 1, translate: [(Q * ac) / d, (S * Z) / z, 0], scale: 1, width: Q / d, height: S / z, zIndex: Math.ceil(100 - s(ac, d) * 100) },
                    aj = { opacity: 0, translate: [(T * ac - (Q * n) / 2.115) * Y.l, (O * Z - (S * n) / 2.115) * Y.t, 0], scale: n * Y.m, width: Q / d, height: S / z, zIndex: Math.ceil(100 - s(ac, d) * 100) };
                p(this)
                    .find("img")
                    .css({ transform: "translate3d(" + ((-Q * ac) / d + j.marginLeft) + "px," + ((-S * Z) / z + j.marginTop) + "px,0px)", width: j.width, height: j.height });
            } else {
                var ag = { opacity: 1, left: (Q * ac) / d, top: (S * Z) / z, width: Q / d, height: S / z, zIndex: Math.ceil(100 - s(ac, d) * 100) },
                    aj = { opacity: 0, left: (T * ac - (Q * n) / 2) * Y.l, top: (O * Z - (S * n) / 2) * Y.t, width: T * Y.m, height: O * Y.m },
                    ai = { left: -((Q * ac) / d) + j.marginLeft, top: -((S * Z) / z) + j.marginTop, width: j.width, height: j.height },
                    ah = { left: -n * ((Q / d) * ac - j.marginLeft) * Y.m, top: -n * ((S / z) * Z - j.marginTop) * Y.m, width: n * j.width * Y.m, height: n * j.height * Y.m };
            }
            if (!M) {
                var aa = ag;
                ag = aj;
        ...
        if (M) {
            p(i).each(function (ac) {
                ...
                var ab = (a * 4) / (d * z);
                if (m.support.transform) {
                    var ad = { opacity: 0, translate: [Q / 2, S / 2, 0], scale: 0, width: Q / d, height: S / z, zIndex: Math.ceil(100 - s(Z, d) * 100) },
                        af = { opacity: 1, translate: [(Q * Z) / d, (S * Y) / z, 0], scale: 1, width: Q / d, height: S / z, zIndex: Math.ceil(100 - s(Z, d) * 100) };
                    p(this)
                        .find("img")
                        .css({ transform: "translate3d(" + ((-Q * Z) / d + j.marginLeft) + "px," + ((-S * Y) / z + j.marginTop) + "px,0px)", width: j.width, height: j.height });
                } else {
    ...
    function F(P, K, O, L) {
        ...
        for (var N = 0; M * N < O; N++) {
            ...
            p(document.createElement("img"))
                .attr("src", P.attr("src"))
                .css({ opacity: 1 / (N / 1.8 + 1), position: "absolute", "z-index": S, left: Math.round(T) + "px", top: Math.round(R) + "px", width: "100%", height: "100%" })
                .appendTo(W);
        }
        return W;
    }
